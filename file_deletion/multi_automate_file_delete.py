...
class FileDelete:
    ...
    def _update_log(self, id, proc_status, file_name, tab_count, error_msg,facility_id):
            conn=connect_to_db.connect('filedb')[0]
            cur = conn.cursor()
            deletion_end_time = self.delete_end_time
            deletion_status_check = proc_status

            update_query = """UPDATE public.file_deletion_log
            ...
            cur.execute(update_query,(deletion_end_time, deletion_status_check, 
                                      tab_count, error_msg, id, facility_id))
            conn.commit()
            cur.close()
    ...
    def delete_encrypted_files(self,facility_id:str):
        try:
            conn = connect_to_db.connect('filedb')[0]
            cur = conn.cursor()
            ...
            for file in files:
                ...
                try:
                    if os.path.exists(encrypted_local_dir):
                        ...
                        self._update_log(encrypted_filelog_id,'success',encryptedjson_file_name,self.count_of_df, 'no errors',self.facility_id)
                    else:
                        logger.error(f"File {encrypted_local_dir} not found")
                        self.delete_end_time = datetime.now()
                        self._update_log(encrypted_filelog_id,'failed',encryptedjson_file_name,self.count_of_df,'file not found',self.facility_id)
                    ...
                    if os.path.exists(decrypted_local_dir):
                        ...
                        self.delete_end_time = datetime.now()
                        self._update_log(decrypted_filelog_id,'success',decryptedjson_file_name,self.count_of_df, 'no errors',self.facility_id)
                    else:
                        logger.error(f"File {decrypted_local_dir} not found")
                        self.delete_end_time = datetime.now()
                        self._update_log(decrypted_filelog_id,'failed',decryptedjson_file_name,self.count_of_df,'file not found',self.facility_id)
