...
class FileLoader:
    ...
    def _retrieve_localdir_from_syncfile(self, run_file):
        cur = self._db_connect_filedb().cursor()
        retrieve_query = f"""
        select id, facility_id,decrypted_file_name 
        from sync_file where decrypted_file_name = '{run_file}'
        """
        cur.execute(retrieve_query)
...
loader = FileLoader()
loader._retrieve_localdir_from_syncfile('hiv_regimen_0_20240201203448.json')
