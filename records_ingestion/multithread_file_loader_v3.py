...
class FileLoader:
    ...
    def _ingest_json_data(self, filedb_conn, staging_conn, file_path, staging_table, dtype=None, parse_dates=None):
        ...
        try:
            ...
            self._update_log(staging_conn, 'failed', file_name, 0, f'Error processing JSON file: {file_name} file is empty')
            self._update_flag_syncfile(filedb_conn, 'failed', -2, 0, f'Error processing JSON file: {encrypted_file_name} file is empty')
            logger.info('Sync File Log updated successfully')
