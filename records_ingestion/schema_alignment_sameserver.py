...
Compares schema between two databases on the SAME PostgreSQL server instance
and automatically applies DDL changes to the destination DB before ingestion runs.

Checks:
  - New tables in source not present in destination  → CREATE TABLE
  - New columns in source not present in destination → ALTER TABLE ... ADD COLUMN
...
from database_connection import schema_alignment_connect as connect_to_db
...
# ──────────────────────────────────────────────
# Applying DDL to destination DB
# ──────────────────────────────────────────────
    ...
    with dest_conn.cursor() as cur:
        for stmt in statements:
            try:
                ...
                raise
        dest_conn.commit()
    logger.info(f"[SCHEMA] {len(statements)} DDL statement(s) applied successfully.")
...
def run_schema_alignment(
    source_db_key: str = "server_a",  # Defaults to your new local server_a routing profile
    dest_db_key: str = "server_b",    # Defaults to your local server_b routing profile
    dry_run: bool = False,
) -> SchemaDiff:
    """
    Compare schemas between two databases hosted on the same server instance, 
    then apply any required DDL changes to the destination database.

    Parameters
    ----------
    source_db_key : str
        The profile lookup key for the source database (e.g., 'server_a').
    dest_db_key : str
        The profile lookup key for the destination database (e.g., 'server_b').
    dry_run : bool
        If True, compute and log the diff but do NOT apply any DDL.
    """
    logger.info(f"[SCHEMA] Starting alignment check on same server. Source Profile: '{source_db_key}', Dest Profile: '{dest_db_key}'")

    # Pass the keys positionally as your connect function expects!
    source_conn = connect_to_db.connect(source_db_key)[0]
    dest_conn = connect_to_db.connect(dest_db_key)[0]

    try:
        logger.info(f"[SCHEMA] Fetching schema from source database configuration ({source_db_key}) …")
        source_schema = fetch_schema(source_conn)

        logger.info(f"[SCHEMA] Fetching schema from destination database configuration ({dest_db_key}) …")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PostgreSQL intra-server schema alignment")
    parser.add_argument("--source", default="server_a", help="Source database key string profile")
    parser.add_argument("--dest", default="server_b", help="Destination database key string profile")
